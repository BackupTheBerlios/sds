...
c  subroutine fpcuro finds the real*8 zeros of a cubic polynomial
...
c
c  input parameters:
c    a,b,c,d: real*8 values, containing the coefficients of p(x).
c
c  output parameters:
c    x      : real*8 array,length 3, which contains the real*8 zeros of p(x)
c    n      : integer, giving the number of real*8 zeros of p(x).
c  ..
c  ..scalar arguments..
      real*8 a,b,c,d
      integer n
...
c  ..function references..
      real*8 abs,dmax1,atan,atan2,cos,sign,sqrt
c  set constants
      two = 0.2e+01
      three = 0.3e+01
      four = 0.4e+01
