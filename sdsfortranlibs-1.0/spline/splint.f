...
c    a,b  : real*8 values, containing the end points of the integration
c           interval. s(x) is considered to be identically zero outside
c           the interval (t(k+1),t(n-k)).
c
c  output parameter:
c    aint : real*8, containing the integral of s(x) between a and b.
c    wrk  : real*8 array, length n.  used as working space
