...
c   u     : real*8 array of dimension at least (m). before entry,
...
c   x     : real*8 array of dimension at least idim*m.
...
c   xx    : real*8 array of dimension at least idim*m.
...
c   w     : real*8 array of dimension at least (m). before entry, w(i)
...
c           unchanged on exit.
c   db    : real*8 array of dimension nb. before entry db(idim*l+j) must
...
c           unchanged on exit.
c   de    : real*8 array of dimension ne. before entry de(idim*l+j) must
...
c           k=1,3 or 5.
c           unchanged on exit.
c   s     : real*8.on entry (in case iopt>=0) s must specify the smoothing
...
c   t     : real*8 array of dimension at least (nest).
...
c   c     : real*8 array of dimension at least (nest*idim).
...
c   cp    : real*8 array of dimension at least 2*(k+1)*idim.
...
c   fp    : real*8. unless ier = 10, fp contains the weighted sum of
c           squared residuals of the spline curve returned.
c   wrk   : real*8 array of dimension at least m*(k+1)+nest*(6+idim+3*k).
...
c  ..array arguments..
      real*8 u(m),x(mx),xx(mx),db(nb),de(ne),w(m),t(nest),c(nc)
      real*8 wrk(lwrk),cp(np)
      integer iwrk(nest)
c  ..local scalars..
      real*8 tol,dist
