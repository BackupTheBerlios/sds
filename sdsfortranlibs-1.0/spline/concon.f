...
c    x   : real*8 array of dimension at least (m). before entry, x(i)
...
c    y   : real*8 array of dimension at least (m). before entry, y(i)
...
c    v   : real*8 array of dimension at least (m). before entry, v(i)
...
c    s   : real*8. on entry s must specify an over-estimate for the
...
c          calls.
c    c   : real*8 array of dimension at least (nest).
c          on succesful exit, this array will contain the coefficients
c          c(1),c(2),..,c(n-4) in the b-spline representation of s(x)
c    sq  : real*8. unless ier>0 , sq contains the weighted sum of
c          squared residuals of the spline approximation returned.
c    sx  : real*8 array of dimension at least m. on exit with ier<=0
...
c          calls.
c   wrk  : real*8 array of dimension at least (m*4+nest*8+maxbin*(maxbin+
