*DECK DCOV
      SUBROUTINE DCOV (FCN, IOPT, M, N, X, FVEC, R, LDR, SIGMA, INFO,
     +WA1, WA2, WA3, WA4)
...
C***MODIFIED BY Roland Schäfer /(FZK-IFP)
C***DESCRIPTION
C
C  1. Purpose.
C
...
C         be less than N.
C
C   SIGMA is the Variance which one has to assigne as error bars to the 
C         M functions to make the least square approximation acceptable
C    ***  In the original code the covariance matrix was scaled by SIGMA.
C         This is not what one allways wants to do. It is only justified if
C         you do not know anything about the error bars of the functions.
C
...
300   CONTINUE
     ...
     +   'EXECUTION TERMINATED BECAUSE USER SET IFLAG NEGATIVE.', 1, 0)
      IF (INFO .EQ. 0) CALL XERMSG ('SLATEC', 'DCOV',
     +   'INVALID INPUT PARAMETER.', 2, 0)
