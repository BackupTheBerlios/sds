...
extern int splder_(doublereal *t, integer *n, doublereal *c, integer *k,
		   integer *nu, doublereal *x, doublereal *y, integer *m,
...
int dsdydx_yyparse(void);
extern FILE *dsdydx_yyin;
extern int dsdydx_yydebug;
extern struct DsdydxParm yyparam;
}
...
void inform_user(double min,		double max,	integer nop,
		 ...
		 integer order,		integer der);
void update_log(int id, int newid,	integer mode,
 		double min,		double max,	integer nop,
		doublereal sfac,	integer iopt,	doublereal fp,
		doublereal *t,		integer n,	integer ier,
		integer order,		integer der,	SharedData *sd);


void splineit(const char *idstr, integer order, integer der, double sfac,
	      integer iopt, double min, double max, integer nop, 
	      integer mode);
char * rl_gets (void);
pid_t fork_qpl(void) ;
pid_t fork_qpl2(void) ;
pid_t pid=0;
pid_t pid2=0;
...
static struct option const long_options[] =
{
	...
	{"derivative-order", required_argument, 0, 'e'},
...
char verstring[] = "$Id: dsdydx.cc,v 1.1 2003/01/23 15:12:06 vstein Exp $";
...
void usage(void)
{
	fprintf(stderr,
		...
		"-e     --derivative-order order of derivative to calculate\n"
		"-m     --sigma        string like"
		                         " \"(0.0,.1|.2|.3),(1.0,.3|1.0)\"\n"
...
int main(int argc,char **argv)
{
	...
	while ((c = getopt_long(argc, argv, "i:a:n:f:k:e:o:m:crRh?VWC",
		...
		case 'o':
			...
			break;
		case 'e':
			if(sscanf(optarg,"%d",&der)!=1)
				usage();
			opt_set|=DER_SET;
			break;
		case 'm':
			dsdydx_yyin=tmpfile();
			fprintf(dsdydx_yyin,"sigma=%s;",optarg);
			rewind(dsdydx_yyin);
			//dsdydx_yydebug=1;
			if(dsdydx_yyparse())
				usage();
	...
	for(i=optind;i<argc;i++)
		splineit(argv[i], order, der, sfac, iopt,
			 min, max, nop, mode);
...
void splineit(const char *idstr, integer order, integer der, 
	      double sfac, integer iopt, double min, double max,
	      integer nop, integer mode)
{
	SharedData *sd;
	int id,newid=DS_UNUSED,newid2=DS_UNUSED;
	integer i,j;
	...
	doublereal *wrk2=(doublereal*)xmalloc(sizeof(doublereal)*lwrk);
...
//interactive call to splineit
	...
	while(mode&INTERAC_MODE) {
		...
		if(newid==DS_UNUSED) {
			...
			sd->set_n(newid,newn);
			sd->add_ident(newid,"auxilary data of dsdydx 1");
		...
		yn=sd->y(newid);
		if(newid2==DS_UNUSED) {
			newid2=sd->create(newn);
			sd->set_n(newid2,newn);
			sd->add_ident(newid2,"auxilary data of dsdydx 2");
			sd->set_plev(newid2,97);
			sd->set_linecolor(newid2,"Gray");
			sd->set_linewidth(newid2,3);
		} else if (newn!=(int)sd->n(newid2)) {
			sd->resize(newid2,newn);
			sd->set_n(newid2,newn);
		}
		xn2=sd->x(newid2);
		yn2=sd->y(newid2);
		...
		if(nop==-1) {
			...
			for(i=0;i<newn;i++)
				xn2[i]=xn[i]=min+(double)i/(double)
					(newn-1)*(max-min);
		}
		splev_(t,&n,c,&order,xn,yn,&newn,&ier1);
		splder_(t,&n,c,&order,&der,xn2,yn2,&newn,wrk2,&ier1);
		if(show_res&&nop==-1)
			for(i=0;i<newn;i++)
				yn[i]=y[i]-yn[i];
...
//non-interactive call to splineit
	...
	if (mode&REP_MODE) {
		...
		x=sd->x(id);
		y=sd->y(id);
		if ((mode&RES_MODE)&&(der==0)) {  // REP_MODE & RES_MODE
			...
			splder_(t,&n,c,&order,&der,x,y,&newn,wrk2,&ier1);
		}
	} else {
		newid=sd->create(newn);
		sd->set_n(newid,newn);
		...
		if ((mode&RES_MODE)&&(der==0)) {  // ~REP_MODE & RES_MODE
			...
			splder_(t,&n,c,&order,&der,xn,yn,&newn,wrk2,&ier1);
		}
	}

	update_log(id, newid, mode, min, max, nop, sfac, 
		   iopt, fp, t, n, ier, order, der, sd);
	sd->changed();
		 ...
		 integer order, integer der)
{
	...
	printf("i=%f, a=%f, n=%ld, f=%f, k=%ld, der=%ld, iopt=%ld\n",
	       min,max,nop,sfac,order,der,iopt);
	print_sigma(stdout);
}
		...
		integer order,		integer der,    SharedData *sd)
{
	...
	off+=asprintcat(&logline,&ll,off,
			" -k %ld -e %ld -f %g -o %ld",order,der,sfac,iopt);
