...
int dswl_yyparse(void);
extern FILE *dswl_yyin;
extern int dswl_yydebug;
...
doublereal wlfit(doublereal *xp, integer np, doublereal x);
doublereal dswlfit(integer i, doublereal *, integer, doublereal x);

#define  FIT(a,b,c)  wlfit(a,b,c)
#define DFIT(a,b,c,d) dswlfit(a,b,c,d)
...
static struct option const long_options[] =
{
	...
	{"B_si"         , required_argument, 0, 301},
	{"b"            , required_argument, 0, 302},
	{"B0"           , required_argument, 0, 303},
	{"G0"           , required_argument, 0, 304},
	{"A?"           , required_argument, 0, 400},
	{"B_si?"        , required_argument, 0, 401},
	{"b?"           , required_argument, 0, 402},
	{"B0?"          , required_argument, 0, 403},
	{"G0?"          , required_argument, 0, 404},
...
void usage(void)
{
	fprintf(stderr,
		...
		"\n"
		"This program tries to fit 2D weak localization to the"
		" datasets matching\n"
		"the regexp(s), which should represent conductance in"
		" units of L_00 as a\n"
		"function of magnetic field. Five parameters are used:\n"
		"G(A,B_si,b,B0,G0;B) = A * 2DWL(B_si,b;B-B0) + G0\n"
		"     b is the ratio of triplet to singulet field;"
		" B_tr=B_si*b\n"
		"     2DWL(B_si,b;x)=1.5*F(0.5+B_si*b/x) - 0.5*F(0.5+B_si/x)\n"
		"                    + log(x) + 0.5*log(B_si) -"
		" 1.5*log(B_si*b)\n"
		"     F is the digamma (or psi) function\n"
		"default values used:\n"
		"A=1, B_si=1, b=1, B0=0, G0=0\n",
...
int main(int argc,char **argv)
{
	...
	while ((c = getopt_long(argc, argv, 
		...
		case 'm':
			dswl_yyin=tmpfile();
			fprintf(dswl_yyin,"sigma=%s;",optarg);
			rewind(dswl_yyin);
			//dsspl_yydebug=1;
			if(dswl_yyparse())
				usage();
	   ...
	   double min, double max, integer nop, int mode)
{
	...
	// 2. IOPT
	// IOPT=1, 2, or 3. See the Macro definition in dswl_y_l.h
	// 3. M
...
doublereal wlfit(doublereal *xp, integer np, doublereal x)
{
	/* weak localization (2D) in units of L00
	   params: 0: prefactor, 1: B_si, 2: B_tr/B_si, 3: x-offset, 4: y-offset */
	doublereal xx=fabs(x-xp[3]);
	doublereal btr=fabs(xp[1]*xp[2]);
	doublereal bsi=fabs(xp[1]);
	doublereal arg1=0.5+btr/xx;
	doublereal arg2=0.5+bsi/xx;
	return xp[0]*(1.5*dpsi_(&arg1)-0.5*dpsi_(&arg2)
		+log(xx)+0.5*log(bsi)-1.5*log(btr))+xp[4];
     
}

#define SIGN(x) (((x)<0)?-1.0:1.0)

/* doublereal dswlfit(integer i, doublereal *xp, integer np, doublereal x) */
doublereal dswlfit(integer i, doublereal *xp, integer, doublereal x)
{
	if (i==4)
		return -1;
	...
	if (i==0)
		return -1.5*dpsi_(&arg1)+0.5*dpsi_(&arg2)
			-log(xx)-0.5*log(bsi)+1.5*log(btr);

	doublereal dpsi1, dpsi2;
	integer one=1;
	integer nz, ierr;

	dpsifn_(&arg1, &one, &one, &one, &dpsi1, &nz, &ierr);
	dpsifn_(&arg2, &one, &one, &one, &dpsi2, &nz, &ierr);
	
	switch (i) {
	case 1:
		return -SIGN(xp[1])*xp[0]*(1.5*dpsi1*fabs(xp[2])/xx
				       - 0.5*dpsi2/xx - 1/bsi);
	case 2:
		return -SIGN(xp[2])*xp[0]*1.5*(dpsi1*bsi/xx 
					   - 1/fabs(xp[2]));
	case 3:
		return -SIGN(x-xp[3])*xp[0]/xx*(1.5*dpsi1*btr/xx
					  - 0.5*dpsi2*bsi/xx - 1);
	}
